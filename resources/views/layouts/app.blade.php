...
<div id="categories-overlay"
    ...
    <div id="categories-content"
        ...
        <div class="flex items-center justify-between border-b border-gray-300 pb-5 mb-16">
            <h2 class="text-4xl font-light tracking-tight text-gray-900">Shop by Category</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-24">
            <div class="flex flex-col gap-8">
                <p class="text-xs font-bold text-gray-400 uppercase tracking-[0.2em]">Our Collections</p>
                <div class="flex flex-col gap-5">
                    <a href="/products?category=tech" class="text-2xl font-medium text-gray-800 hover:text-blue-600 transition">Tech</a>
                    <a href="/products?category=fashion" class="text-2xl font-medium text-gray-800 hover:text-blue-600 transition">Fashion</a>
                    <a href="/products?category=home" class="text-2xl font-medium text-gray-800 hover:text-blue-600 transition">Home</a>
                </div>
            </div>

            <div class="bg-gray-50/50 p-8 rounded-3xl border border-gray-100 h-fit">
                <p class="text-sm font-semibold text-gray-900 mb-2">Curated Spaces</p>
                <p class="text-xs text-gray-500 leading-relaxed mb-4">Explore minimal hardware, premium garments, and essential products designed for daily utility.</p>
                <a href="/products" class="text-blue-600 text-xs font-bold hover:underline">View all categories →</a>
            </div>
        </div>
    </div>
</div>

<nav class="bg-white/70 backdrop-blur-md border-b border-gray-200/80 sticky top-0 z-50 text-[13px] tracking-tighter font-normal text-gray-800">
    <div class="max-w-[1024px] mx-auto px-6 h-12 flex items-center justify-between">

        <div class="w-1/4 flex justify-start">
            <a href="/products" class="text-sm font-semibold tracking-tight text-black hover:opacity-70 transition-opacity">
                CartForge
            </a>
        </div>

        <div class="w-2/4 flex justify-center items-center gap-10 text-gray-500 font-medium">
            <a href="/products" class="hover:text-black transition-colors">Home</a>
            <button id="categories-btn" class="hover:text-black transition-colors focus:outline-none">Categories</button>
        </div>
        ...
        <div class="w-1/4 flex justify-end items-center gap-6">
            ...
            <button id="search-btn"
                ...
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.7" stroke="currentColor" class="w-4 h-4">
                    <path stroke-linecap="round" stroke-linejoin="round" d="m21 21-4.35-4.35m1.85-5.15 a7 7 0 1 1-14 0 a7 7 0 0 1 14 0Z"/>
                </svg>
            </button>

            @auth
                <a href="{{ route('cart.index') }}" class="relative hover:scale-105 transition">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.7" stroke="currentColor" class="w-4 h-4 text-black/80">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 3h1.386a1.5 1.5 0 0 1 1.415 1.022L5.383 5.25m0 0h13.867 a1.5 1.5 0 0 1 1.464 1.825l-1.5 7.5 a1.5 1.5 0 0 1-1.464 1.175H8.239 a1.5 1.5 0 0 1-1.464-1.175L5.383 5.25Zm3.367 13.5a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm8.25 0a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z"/>
                    </svg>
                    <span id="cart-count" class="absolute -top-1.5 -right-2 min-w-[14px] h-[14px] text-[9px] bg-gray-900 text-white flex items-center justify-center rounded-full font-bold">
                        {{ $cartCount ?? 0 }}
                    </span>
                </a>
                ...
                <form method="POST" action="{{ route('logout') }}" class="flex items-center">
                    @csrf
                    <button type="submit" title="Logout" class="hover:scale-105 active:scale-95 transition text-gray-700 hover:text-red-600">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.7" stroke="currentColor" class="w-4 h-4">
            ...
            @else
                <a href="/login" title="Login" class="hover:scale-105 active:scale-95 transition text-gray-700 hover:text-blue-600">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.7" stroke="currentColor" class="w-4 h-4 text-black/80">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 3h1.386a1.5 1.5 0 0 1 1.415 1.022L5.383 5.25m0 0h13.867 a1.5 1.5 0 0 1 1.464 1.825l-1.5 7.5 a1.5 1.5 0 0 1-1.464 1.175H8.239 a1.5 1.5 0 0 1-1.464-1.175L5.383 5.25Zm3.367 13.5a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm8.25 0a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z"/>
                    </svg>
                </a>
